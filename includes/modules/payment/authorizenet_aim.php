...
class authorizenet_aim extends base
{
     ...
     * debug content var
     */
    public $reportable_submit_data = array();
    /**
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if ($this->enabled) {
                ...
                if (!function_exists('curl_init')) {
                    $messageStack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_ERROR_CURL_NOT_FOUND, 'error');
                }
                if (strlen(MODULE_PAYMENT_AUTHORIZENET_AIM_MD5HASH) > 20) {
                    $this->title .= '<span class="alert"> (NOTE: MD5 Hash key too long)</span>';
                }
            }
        } else {
    ...
    public function update_status()
    {
        ...
        if (IS_ADMIN_FLAG === false) {
            ...
            if (MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE == 'Prduction' && (!defined('ENABLE_SSL') || ENABLE_SSL != 'true')) {
                $this->enabled = false;
            }
        }
    ...
    // Create a string that contains a listing of products ordered for the description field
        ...
        if (sizeof($this->submit_extras)) {
            $submit_data = array_merge($submit_data, $this->submit_extras);
        }
     ...
     */
    public function _sendRequest($submit_data)
    {
        global $request_type;
      ...
      case 'AIM':
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE == 'Sandbox') {
            $url = $devurl;
        }
        if (defined('AUTHORIZENET_DEVELOPER_MODE') && AUTHORIZENET_DEVELOPER_MODE == 'certify') {
            $url = $certurl;
        }
      break;
    }
        ...
        if (isset($this->reportable_submit_data['x_exp_date'])) {
            $this->reportable_submit_data['x_exp_date'] = '****';
        }
     ...
     */
    public function calc_md5_response($trans_id = '', $amount = '')
    {
        if ($amount == '' || $amount == '0') {
            $amount = '0.00';
        }
     ...
     */
    public function _debugActions($response, $order_time= '', $sessID = '')
    {
        global $db;
        if ($order_time == '') {
            $order_time = date("F j, Y, g:i a");
