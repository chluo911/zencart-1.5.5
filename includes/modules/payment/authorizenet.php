...
class authorizenet extends base
{
    ...
    public function __construct()
    {
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_AUTHORIZATION_TYPE == 'Authorize') {
            $this->order_status = 1;
        }
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_TESTMODE == 'Sandbox') {
...
//     $this->form_action_url = 'https://www.eprocessingnetwork.com/cgi-bin/an/order.pl';
        ...
        if (AUTHORIZENET_DEVELOPER_MODE == 'echo' || MODULE_PAYMENT_AUTHORIZENET_DEBUGGING == 'echo') {
            $this->form_action_url = 'https://developer.authorize.net/param_dump.asp';
        }
     ...
     * @return string
     */
    public function InsertFP($loginid, $txnkey, $amount, $sequence, $currency = "")
    {
        $tstamp = time();
...
//       && $this->authorize['x_description'] == $this->submit_data['x_description']
     ...
     */
    public function _debugActions($response, $mode, $order_time= '', $sessID = '')
    {
