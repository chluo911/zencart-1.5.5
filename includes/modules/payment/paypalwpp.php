...
class paypalwpp extends base
{
    ...
    public function __construct()
    {
        ...
        if (PROJECT_VERSION_MAJOR != '1' && substr(PROJECT_VERSION_MINOR, 0, 3) != '5.5') {
            $this->enabled = false;
        }
...
//    $this->zcLog('update_status', 'Checking whether module should be enabled or not.');
    ...
    public function process_button()
    {
        ...
        if ($this->use_incontext_checkout == false) {
            return '';
        }
    ...
    public function before_process()
    {
        ...
        if (MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE != 'Auth Only' && MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE != 'Sale' && $options['PAYMENTACTION'] == 'Sale' && defined('MODULE_PAYMENT_PAYPALEC_ALLOWEDPAYMENT') && MODULE_PAYMENT_PAYPALEC_ALLOWEDPAYMENT == 'Instant Only') {
        ...
        if ($order->delivery['street_address'] != '' && $order->delivery['street_address'] != $_SESSION['paypal_ec_payer_info']['ship_street_1'] && $_SESSION['paypal_ec_payer_info']['ship_street_1'] != '') {
            ...
            if (($address_arr = $this->getOverrideAddress()) !== false) {
                ...
                if ($address_arr['entry_suburb'] != '') {
                    $options['PAYMENTREQUEST_0_SHIPTOSTREET2'] = $address_arr['entry_suburb'];
                }
        ...
        if (isset($options['PAYMENTREQUEST_0_SHIPTOSTREET2']) && trim($options['PAYMENTREQUEST_0_SHIPTOSTREET2']) == '') {
            unset($options['PAYMENTREQUEST_0_SHIPTOSTREET2']);
        }
        if (isset($options['PAYMENTREQUEST_0_SHIPTOPHONENUM']) && trim($options['PAYMENTREQUEST_0_SHIPTOPHONENUM']) == '') {
            unset($options['PAYMENTREQUEST_0_SHIPTOPHONENUM']);
        }

        // if State is not supplied, repeat the city so that it's not blank, otherwise PayPal croaks
        if ((!isset($options['PAYMENTREQUEST_0_SHIPTOSTATE']) || trim($options['PAYMENTREQUEST_0_SHIPTOSTATE']) == '') && $options['PAYMENTREQUEST_0_SHIPTOCITY'] != '') {
...
//      $order->info['total'] = zen_round($order->info['total'], 2);
        ...
        if (isset($options['PAYMENTREQUEST_0_DESC']) && $options['PAYMENTREQUEST_0_DESC'] == '') {
            unset($options['PAYMENTREQUEST_0_DESC']);
        }
        if (!isset($options['PAYMENTREQUEST_0_AMT'])) {
            $options['PAYMENTREQUEST_0_AMT'] = number_format($order_amount, 2, '.', '');
        }
        ...
        $response = $doPayPal->DoExpressCheckoutPayment(
        ...
        if ($response['PAYMENTINFO_0_PAYMENTTYPE'] != '') {
            $this->payment_type .=  ' (' . urldecode($response['PAYMENTINFO_0_PAYMENTTYPE']) . ')';
        }
        ...
        if (empty($response['PAYMENTINFO_0_PENDINGREASON']) ||
          ...
          $response['PAYMENTINFO_0_PAYMENTSTATUS'] == 'Completed') {
            ...
            if ($response['PAYMENTINFO_0_PAYMENTSTATUS'] == 'Pending') {
                if ($response['L_ERRORCODE0'] == 11610 && $response['PAYMENTINFO_0_PENDINGREASON'] == '') {
                    $response['PAYMENTINFO_0_PENDINGREASON'] = 'Pending FMF Review by Storeowner';
                }
            }
    ...
    public function admin_notification($zf_order_id)
    {
        ...
        if (file_exists(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/paypal/paypalwpp_admin_notification.php')) {
     ...
     */
    public function _GetTransactionDetails($oID, $last_txn = true)
    {
        if ($oID == '' || $oID < 1) {
            return false;
        }
     ...
     */
    public function _TransactionSearch($startDate = '', $oID = '', $criteria = '')
    {
        global $db, $messageStack, $doPayPal;
        $doPayPal = $this->paypal_init();
        // look up history on this order from PayPal table
        ...
        $timeval = time();
        if ($startDate == '') {
            $startDate = date('Y-m-d', $timeval) . 'T' . date('h:i:s', $timeval) . 'Z';
        }
        /**
         * Read data from PayPal
         */
     ...
     */
    public function zcLog($stage, $message)
    {
        static $tokenHash;
        if ($tokenHash == '') {
     ...
     */
    public function _doDebug($subject = 'PayPal debug data', $data, $useSession = true)
    {
    ...
    public function _doRefund($oID, $amount = 'Full', $note = '')
    {
        ...
        if ($doPayPal->_mode == 'payflow' && $refundAmt == 'Full') {
            $refundAmt = $PFamt;
        }
     ...
     */
    public function setStateAndCountry(&$info)
    {
        global $db, $messageStack;
    ...
    public function getLineItemDetails($restrictedCurrency)
    {
        ...
        if (sizeof($order_totals)) {
            ...
            for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
                ...
                if (in_array($order_totals[$i]['code'], array('ot_total','ot_subtotal','ot_tax','ot_shipping')) || strstr($order_totals[$i]['code'], 'insurance')) {
                    if ($order_totals[$i]['code'] == 'ot_shipping') {
                        $optionsST['PAYMENTREQUEST_0_SHIPPINGAMT'] = round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_total') {
                        $optionsST['PAYMENTREQUEST_0_AMT']         = round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_tax') {
                        $optionsST['PAYMENTREQUEST_0_TAXAMT']     += round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_subtotal') {
                        $optionsST['PAYMENTREQUEST_0_ITEMAMT']     = round($order_totals[$i]['value'], 2);
                    }
                    if (strstr($order_totals[$i]['code'], 'insurance')) {
                        $optionsST['PAYMENTREQUEST_0_INSURANCEAMT'] += round($order_totals[$i]['value'], 2);
                    }
            ...
            if (sizeof($this->ot_merge)) {
                $optionsST = array_merge($optionsST, $this->ot_merge);
            }

            if ($creditsApplied > 0) {
                $optionsST['PAYMENTREQUEST_0_ITEMAMT'] -= $creditsApplied;
            }
            if ($surcharges > 0) {
                $optionsST['PAYMENTREQUEST_0_ITEMAMT'] += $surcharges;
            }
...
//    }
        ...
        for ($k=0, $n=$numberOfLineItemsProcessed+1; $k<$n; $k++) {
            if (isset($optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"])) {
                unset($optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"]);
            }
        }
        ...
        if ((!isset($optionsST['PAYMENTREQUEST_0_AMT']) || $optionsST['PAYMENTREQUEST_0_AMT'] == 0 || $flagSubtotalsUnknownYet == true || $optionsST['PAYMENTREQUEST_0_ITEMAMT'] == 0) && $discountProblemsFlag != true) {
            ...
            if ($subTotalShipping > 0) {
                $optionsST['PAYMENTREQUEST_0_SHIPPINGAMT'] = $subTotalShipping;
            }
        ...
        if (isset($optionsST['PAYMENTREQUEST_0_ITEMAMT']) && $optionsST['PAYMENTREQUEST_0_ITEMAMT'] == 0) {
            unset($optionsST['PAYMENTREQUEST_0_ITEMAMT']);
        }
        if (isset($optionsST['PAYMENTREQUEST_0_TAXAMT']) && $optionsST['PAYMENTREQUEST_0_TAXAMT'] == 0) {
            unset($optionsST['PAYMENTREQUEST_0_TAXAMT']);
        }
        if (isset($optionsST['PAYMENTREQUEST_0_SHIPPINGAMT']) && $optionsST['PAYMENTREQUEST_0_SHIPPINGAMT'] == 0) {
            unset($optionsST['PAYMENTREQUEST_0_SHIPPINGAMT']);
        }
        if (isset($optionsST['PAYMENTREQUEST_0_SHIPDISCAMT']) && $optionsST['PAYMENTREQUEST_0_SHIPDISCAMT'] == 0) {
            unset($optionsST['PAYMENTREQUEST_0_SHIPDISCAMT']);
        }
        if (isset($optionsST['PAYMENTREQUEST_0_HANDLINGAMT']) && $optionsST['PAYMENTREQUEST_0_HANDLINGAMT'] == 0) {
            unset($optionsST['PAYMENTREQUEST_0_HANDLINGAMT']);
        }
        if (isset($optionsST['PAYMENTREQUEST_0_INSURANCEAMT']) && $optionsST['PAYMENTREQUEST_0_INSURANCEAMT'] == 0) {
            unset($optionsST['PAYMENTREQUEST_0_INSURANCEAMT']);
        }
    ...
    public function ec_step1()
    {
        ...
        if (MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE == 'Order') {
            $options['PAYMENTREQUEST_0_PAYMENTACTION'] = 'Order';
        }
        // Allow delayed payments such as eCheck? (can only use InstantPayment if Action is Sale)
        if (MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE != 'Auth Only' && MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE != 'Sale' && $options['PAYMENTACTION'] == 'Sale' && defined('MODULE_PAYMENT_PAYPALEC_ALLOWEDPAYMENT') && MODULE_PAYMENT_PAYPALEC_ALLOWEDPAYMENT == 'Instant Only') {
        ...
        if ($this->use_incontext_checkout && $options['SOLUTIONTYPE'] == 'Sole') {
            $options['SOLUTIONTYPE'] = 'Mark';
        }
        ...
        if ($_SESSION['cart']->get_content_type() == 'virtual' && isset($_SESSION['customer_id']) && $_SESSION['customer_id'] > 0) {
            ...
            if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS != 'Yes' && ($address_arr = $this->getOverrideAddress()) !== false) {
                ...
                if ($address_error == false) {
                    ...
                    if ($address_arr['entry_suburb'] != '') {
                        $options['PAYMENTREQUEST_0_SHIPTOSTREET2'] = substr($address_arr['entry_suburb'], 0, 100);
                    }
...
//      echo ' newAMT:'.$amt;
    ...
    public function ec_step2()
    {
        ...
        if ($_SESSION['cart']->get_content_type('true') > 0 || in_array($_SESSION['cart']->content_type, array('mixed', 'virtual'))) {
...
//    } else {
        ...
        if ($response['PAYMENTREQUEST_0_SHIPTOSTREET2'] == $response['PAYMENTREQUEST_0_SHIPTOSTREET1']) {
            $response['PAYMENTREQUEST_0_SHIPTOSTREET2'] = '';
        }

        // accomodate PayPal bug which incorrectly treats 'Yukon Territory' as YK instead of ISO standard of YT.
        if ($response['PAYMENTREQUEST_0_SHIPTOSTATE'] == 'YK') {
            $response['PAYMENTREQUEST_0_SHIPTOSTATE'] = 'YT';
        }
        // same with Newfoundland
        if ($response['PAYMENTREQUEST_0_SHIPTOSTATE'] == 'NF') {
            $response['PAYMENTREQUEST_0_SHIPTOSTATE'] = 'NL';
        }
...
//    }
        ...
        if ($order->info['total'] < 0.01 && urldecode($response['PAYMENTREQUEST_0_AMT']) > 0) {
            $order->info['total'] = urldecode($response['PAYMENTREQUEST_0_AMT']);
        }
     ...
     */
    public function ec_step2_finish($paypal_ec_payer_info, $new_acct_notify)
    {
        global $db, $order;
            ...
            'entry_country_id'          => $country_id);
                ...
                if ($this->new_acct_notify == 'Yes') {
                    ...
                    if (trim(EMAIL_SUBJECT) != 'n/a') {
                        zen_mail($paypal_ec_payer_info['payer_firstname'] . " " . $paypal_ec_payer_info['payer_lastname'], $paypal_ec_payer_info['payer_email'], EMAIL_SUBJECT, $email_text, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $email_html, 'welcome');
                    }
    ...
    /**
       ...
       * @return int|boolean
       */
    public function findMatchingAddressBookEntry($customer_id, $address_question_arr)
    {
        global $db;

        // if address is blank, don't do any matching
        if ($address_question_arr['street_address'] == '') {
            return false;
        }
...
//        $fromDb = str_replace("\n", '', $answers_arr->fields['entry_company']);
    ...
    /**
       ...
       * @return int
       */
    public function addAddressBookEntry($customer_id, $address_question_arr, $make_default = false)
    {
        global $db;
        ...
        if ($address_question_arr['company'] != '' && $address_question_arr['company'] != $address_question_arr['name']) {
            array('fieldName'=>'entry_company', 'value'=>$address_question_arr['company'], 'type'=>'string');
        }
     ...
     */
    public function user_login($email_address, $redirect = true)
    {
     ...
     */
    public function ec_delete_user($cid)
    {
        global $db;
     ...
     */
    public function terminateEC($error_msg = '', $kill_sess_vars = false, $goto_page = '')
    {
        global $messageStack, $order, $order_total_modules;
        $error_msg = trim($error_msg);
        if (substr($error_msg, -1) == '-') {
            $error_msg = trim(substr($error_msg, 0, strlen($error_msg) - 1));
        }
        $stackAlert = 'header';
     ...
     */
    public function _errorHandler($response, $operation = '', $ignore_codes = '')
    {
      ...
      case 'SetExpressCheckout':
        if ($basicError) {
            ...
            if ($detailedEmailMessage != '') {
                zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $this->uncomment($errorNum) . ')', $this->uncomment($detailedEmailMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array('EMAIL_MESSAGE_HTML'=>$this->uncomment($detailedMessage)), 'paymentalert');
            }
      ...
      default:
        if ($basicError) {
            ...
            if ($detailedEmailMessage != '') {
                zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $this->uncomment($errorNum) . ')', $this->uncomment($detailedMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, array('EMAIL_MESSAGE_HTML'=>nl2br($this->uncomment($detailedEmailMessage))), 'paymentalert');
            }
