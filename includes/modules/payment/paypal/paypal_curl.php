...
class paypal_curl extends base
{
   ...
   */
     ...
     */
    public $_curlOptions = array(CURLOPT_HEADER => 0,
    ...
    public function SetExpressCheckout($returnUrl, $cancelUrl, $options = array())
    {
        ...
        if ($this->_mode == 'payflow') {
            ...
            if (!isset($values['PAYMENTREQUEST_0_PAYMENTACTION']) || ($this->checkHasApiCredentials() === false)) {
                $values['PAYMENTREQUEST_0_PAYMENTACTION'] = ($this->_trxtype == 'S' || ($this->checkHasApiCredentials() === false) ? 'Sale' : 'Authorization');
            }
        ...
        if (defined('MODULE_PAYMENT_PAYPALWPP_HEADER_IMAGE')) {
            $values['HDRIMG'] = urlencode(MODULE_PAYMENT_PAYPALWPP_HEADER_IMAGE);
        }
    ...
    public function DoExpressCheckoutPayment($token, $payerId, $options = array())
    {
        $values = array_merge($options, array('TOKEN'   => $token,
                                          'PAYERID' => $payerId));
        if (PAYPAL_DEV_MODE == 'true') {
            $this->log('DoExpressCheckout - breakpoint 1 - ['.$token  . ' ' . $payerId . ' ' . "]\n\n[" . print_r($values, true) .']', $token);
        }
     ...
     */
    public function DoDirectPayment($cc, $cvv2 = '', $exp, $fname = null, $lname = null, $cc_type, $options = array(), $nvp = array())
    {
        $values = $options;
        $values['ACCT'] = $cc;
        if ($cvv2 != '') {
     ...
     */
    public function ManagePendingTransactionStatus($txnID, $action)
    {
        if (!in_array($action, array('Accept', 'Deny'))) {
            return false;
        }
        $values['TRANSACTIONID'] = $txnID;
     ...
     */
    public function _request($values, $operation, $requestId = null)
    {
        ...
        // do debug/logging
        if ((!in_array($operation, array('GetTransactionDetails','TransactionSearch'))) || (in_array($operation, array('GetTransactionDetails','TransactionSearch')) && !strstr($response, '&ACK=Success'))) {
            $this->_logTransaction($operation, $this->_getElapsed($start), $response, $errors . ($commErrNo != 0 ? "\n" . print_r($commInfo, true) : ''));
        }
     ...
     * the transaction.
     */
    public function _buildNameValueList($pairs)
    {
        ...
        foreach ($pairs as $name => $value) {
            ...
            if (($this->_mode == 'payflow') && (strpos($value, '&') !== false || strpos($value, '=') !== false)) {
                ...
                if ($this->_mode == 'nvp' && ((strstr($name, 'SHIPTO') || strstr($name, 'L_NAME') || strstr($name, 'L_PAYMENTREQUEST_0_NAME') || strstr($name, '_DESC')) && (strpos($value, '&') !== false || strpos($value, '=') !== false))) {
     ...
     */
    public function _parseNameValueList($string)
    {
     ...
     */
    public function _logTransaction($operation, $elapsed, $response, $errors)
    {
     ...
     */
    public function _sanitizeLog($log, $allsensitive = false)
    {
        if (is_array($log)) {
          ...
          case 'solutiontype':
            ...
            break;
                ...
                if ($allsensitive && in_array($key, array('BUTTONSOURCE', 'VERSION', 'SIGNATURE', 'USER', 'VENDOR', 'PARTNER', 'PWD', 'VERBOSITY'))) {
                    unset($log[$key]);
                }
            }
    ...
    public function log($message, $token = '')
    {
        static $tokenHash;
        if ($tokenHash == '') {
     ...
     * @return boolean
     */
    public function checkHasApiCredentials()
    {
