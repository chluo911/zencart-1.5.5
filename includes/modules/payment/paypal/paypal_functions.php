...
// Functions for paypal processing
  ...
  function ipn_debug_email($message, $email_address = '', $always_send = false, $subjecttext = 'IPN DEBUG message')
  {
      ...
      if ((defined('MODULE_PAYMENT_PAYPAL_IPN_DEBUG') && (MODULE_PAYMENT_PAYPAL_IPN_DEBUG == 'Log and Email' || MODULE_PAYMENT_PAYPAL_IPN_DEBUG == 'Log File' || MODULE_PAYMENT_PAYPAL_IPN_DEBUG == 'Yes')) || (defined('MODULE_PAYMENT_PAYPALWPP_DEBUGGING') && (MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log File' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email'))) {
          $logfile = ipn_add_error_log($message, $paypal_instance_id);
      }
      return $logfile;
  }
          ...
          if ($ipn_id->RecordCount() <= 0) {
            ...
            case 'address':
              ...
              if ($postArray['payment_status'] == 'Completed') {
                  $transType = 'cleared-address';
              }
              if ($postArray['payment_status'] == 'Denied') {
                  $transType = 'denied-address';
              }
              if ($postArray['payment_status'] == 'Pending') {
            ...
            case 'multi_currency':
              ...
              if ($postArray['payment_status'] == 'Completed') {
                  $transType = 'cleared-multicurrency';
              }
              if ($postArray['payment_status'] == 'Denied') {
                  $transType = 'denied-multicurrency';
              }
              if ($postArray['payment_status'] == 'Pending') {
            ...
            case 'echeck':
              ...
              if ($postArray['payment_status'] == 'Completed') {
                  $transType = 'cleared-echeck';
              }
              if ($postArray['payment_status'] == 'Completed' && $postArray['txn_type'] == 'web_accept') {
                  $transType = 'cleared-echeck';
              }
              if ($postArray['payment_status'] == 'Denied') {
                  $transType = 'denied-echeck';
              }
              if ($postArray['payment_status'] == 'Failed') {
                  $transType = 'failed-echeck';
              }
              if ($postArray['payment_status'] == 'Pending') {
            ...
            case 'authorization':
              ...
              if ($postArray['payment_status'] == 'Voided') {
                  $transType = 'voided';
              }
              if ($postArray['payment_status'] == 'Pending') {
                  $transType = 'pending-authorization';
              }
              if ($postArray['payment_status'] == 'Captured') {
                  $transType = 'captured';
              }
              if ($postArray['payment_status'] == 'Completed') {
                  $transType = 'cleared-authorization';
              }
              if ($postArray['auth_status'] == 'In_Progress') {
                  $transType = 'partial-authorization';
              }
            break;
            case 'verify':
            ...
            case 'paymentreview':
              ...
              if ($postArray['payment_status'] == 'Completed') {
                  $transType = 'cleared-review';
              }
            break;
            case 'intl':
              ipn_debug_email('IPN NOTICE :: Found pending-intl record in database');
              if ($postArray['payment_status'] == 'Completed') {
                  $transType = 'cleared-intl';
              }
              if ($postArray['payment_status'] == 'Denied') {
                  $transType = 'denied-intl';
              }
              if ($postArray['payment_status'] == 'Pending') {
 ...
 */
  function ipn_validate_transaction($info, $postArray, $mode='IPN')
  {
      ...
      if (defined('MODULE_PAYMENT_PAYPAL_BUSINESS_ID')) {
          ...
          if (strtolower(trim($postArray['receiver_email'])) == strtolower(trim(MODULE_PAYMENT_PAYPAL_BUSINESS_ID))) {
              $ppRecEmail = true;
          }
 ...
 */
  function ipn_determine_txn_type($postArray, $txn_type = 'unknown')
  {
      ...
      if ($postArray['txn_type'] == 'send_money') {
          return $postArray['txn_type'];
      }
      if ($postArray['txn_type'] == 'express_checkout' || $postArray['txn_type'] == 'cart') {
 ...
 */
  ...
  function doPayPalIPNFsockopenPostback($web, $postback, $postback_array, $ssl, $mode = 'IPN')
  {
      ...
      $status = '';
      if ($status == '' && substr($firstline, 0, 8) == 'VERIFIED') {
          $status = 'VERIFIED';
      }
      if ($status == '' && substr($firstline, 0, 7) == 'SUCCESS') {
          $status = 'SUCCESS';
      }
      if ($status == '' && substr($firstline, 0, 4) == 'FAIL') {
          $status = 'FAIL';
      }
      if ($status == '' && substr($firstline, 0, 7) == 'INVALID') {
          $status = 'INVALID';
      }
      if ($status == '' && substr($firstline, 0, 12) == 'UNDETERMINED') {
          $status = 'UNDETERMINED';
      }
