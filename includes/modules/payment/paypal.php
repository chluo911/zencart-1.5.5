...
class paypal extends base
{
    ...
    /**
      ...
      * @return paypal
      */
    public function __construct($paypal_ipn_id = '')
    {
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if (IS_ADMIN_FLAG === true && defined('MODULE_PAYMENT_PAYPAL_IPN_DEBUG') && MODULE_PAYMENT_PAYPAL_IPN_DEBUG != 'Off') {
                $this->title .= '<span class="alert"> (debug mode active)</span>';
            }
            if (IS_ADMIN_FLAG === true && MODULE_PAYMENT_PAYPAL_TESTING == 'Test') {
                $this->title .= '<span class="alert"> (dev/test mode active)</span>';
            }
        } else {
...
//                   'paypal_order_id' => $paypal_order_id,
    ...
    public function check()
    {
        ...
        if (IS_ADMIN_FLAG === true) {
            global $sniffer;
            if ($sniffer->field_exists(TABLE_PAYPAL, 'zen_order_id')) {
                $db->Execute("ALTER TABLE " . TABLE_PAYPAL . " CHANGE COLUMN zen_order_id order_id int(11) NOT NULL default '0'");
            }
        }
    ...
    public function _getPDTresults($orderAmount, $my_currency, $pdtTX)
    {
        global $db;
