...
class firstdata_hco extends base
{
    ...
    public function __construct()
    {
        ...
        if (MODULE_PAYMENT_FIRSTDATA_PAYMENTPAGES_AUTHORIZATION_TYPE == 'Authorize') {
            $this->order_status = 1;
        }
        ...
        if (MODULE_PAYMENT_FIRSTDATA_PAYMENTPAGES_TESTMODE == 'Sandbox') {
            $this->form_action_url = 'https://demo.globalgatewaye4.firstdata.com/payment';
        }
...
//       'x_method' => 'CC', // if not passed, then the payment types can be configured in the PaymentPage including enabling PayPal and other features.
            ...
            for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
                ...
                if (in_array($order_totals[$i]['code'], array('ot_total','ot_subtotal','ot_tax','ot_shipping', 'insurance'))) {
                    if ($order_totals[$i]['code'] == 'ot_shipping') {
                        $submit_data_core['x_freight'] = round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_tax') {
                        $submit_data_core['x_tax'] = round($order_totals[$i]['value'], 2);
                    }
                } else {
                    // handle credits
        ...
        if ($order->info['currency'] != $this->gateway_currency) {
            ...
            if (isset($submit_data_core['x_freight'])) {
                $submit_data_core['x_freight'] = number_format($submit_data_core['x_freight'] * $exchange_factor, 2);
            }
            if (isset($submit_data_core['x_tax'])) {
                $submit_data_core['x_tax'] = number_format($submit_data_core['x_tax'] * $exchange_factor, 2);
            }
            if (isset($submit_data_core['discount_amount'])) {
                $submit_data_core['discount_amount'] = number_format($submit_data_core['discount_amount'] * $exchange_factor, 2);
            }
        ...
        if (sizeof($this->submit_extras)) {
            $submit_data_core = array_merge($submit_data_core, $this->submit_extras);
        }
...
//       'x_relay_URL' => zen_href_link(FILENAME_CHECKOUT_PROCESS, 'action=confirm', 'SSL', true, false),
        ...
        if (MODULE_PAYMENT_FIRSTDATA_PAYMENTPAGES_TESTMODE == 'Test') {
