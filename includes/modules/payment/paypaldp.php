 ...
 */
if (!defined('MODULE_PAYMENT_CARDINAL_CENTINEL_DEBUGGING')) {
    define('MODULE_PAYMENT_CARDINAL_CENTINEL_DEBUGGING', false);
}
/**
...
class paypaldp extends base
{
     ...
     * @var string
     */
    public $buttonSource = 'ZenCart-DP_us';
    /**
...
//    $this->zcLog('update_status', 'Checking whether module should be enabled or not.');
    ...
    public function selection()
    {
        ...
        if (sizeof($this->cards)>0) {
            $fieldsArray[] = array('title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_TYPE,
    ...
    public function pre_confirmation_check()
    {
        ...
        $result = $cc_validation->validate(
            $_POST['paypalwpp_cc_number'],
            $_POST['paypalwpp_cc_expires_month'],
            $_POST['paypalwpp_cc_expires_year'],
            (isset($_POST['paypalwpp_cc_issue_month']) ? $_POST['paypalwpp_cc_issue_month'] : ''),
            (isset($_POST['paypalwpp_cc_issue_year']) ? $_POST['paypalwpp_cc_issue_year'] : '')
        );
        $error = '';
        switch ($result) {
      case 1:
      ...
      if ($_POST['paypalwpp_cc_number'] == '') {
          $error = str_replace('\n', '', MODULE_PAYMENT_PAYPALDP_TEXT_JS_CC_NUMBER);
      } // yes, those are supposed to be single-quotes.
      break;
      case -2:
      case -3:
      case -4:
      ...
      break;
        ...
        if (isset($_POST['paypalwpp_cc_issuenumber'])) {
            $_POST['paypalwpp_cc_issuenumber'] = preg_replace('/[^0-9]/i', '', $_POST['paypalwpp_cc_issuenumber']);
        }
        ...
        if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK' && (!isset($_POST['MD']))) {
            ...
            if (isset($_SESSION['3Dsecure_requires_lookup']) && $_SESSION['3Dsecure_requires_lookup'] == true) {
                ...
                if (isset($lookup_response['EciFlag'])) {
                    $_SESSION['3Dsecure_auth_eci'] = $lookup_response['EciFlag'];
                }
          ...
          ////////////////////////////////////////////////////////////////////////
                    ...
                    if ($shouldContinue != 'Y') {
                        ...
                        if (in_array($errorNo, array('8000', '8010', '8020', '8030'))) {
                            $error = CENTINEL_PROCESSING_ERROR;
                        }
                  ...
                  case 'JCB':
                    ...
                    break;
                            ...
                            } elseif ('U' == $_SESSION['3Dsecure_enrolled'] || '0' != $errorNo) {
                                switch ($_SESSION['3Dsecure_card_type']) {
                  case 'VISA':
    ...
    public function before_process()
    {
        ...
        include(DIR_WS_CLASSES . 'cc_validation.php');
        $cc_validation = new cc_validation();
        $response = $cc_validation->validate(
            $_POST['wpp_cc_number'],
            $_POST['wpp_cc_expdate_month'],
            $_POST['wpp_cc_expdate_year'],
            $_POST['wpp_cc_issuedate_month'],
...
//       $optionsAll['SOFTDESCRIPTORCITY'] = substr(preg_replace('/[^a-zA-Z0-9. !,' . preg_quote('"$%&\'()+-*/:;<=>?@') . ']/', '', STORE_TELEPHONE_CUSTSERVICE), 0, 23);
        ...
        $response = $doPayPal->DoDirectPayment(
            $cc_number,
            $cc_checkcode,
     ...
     */
    public function selectCurrency($val = '')
    {
            ...
            for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
                ...
                if (in_array($order_totals[$i]['code'], array('ot_total','ot_subtotal','ot_tax','ot_shipping')) || strstr($order_totals[$i]['code'], 'insurance')) {
                    ...
                    if ($order_totals[$i]['code'] == 'ot_tax') {
                        $optionsST['TAXAMT']     += strval(round($order_totals[$i]['value'], 2));
                    }
...
//    }
        ...
        if ($basicError ||
           ((isset($_SESSION['paypal_ec_token']) && isset($response['TOKEN'])) && $_SESSION['paypal_ec_token'] != urldecode($response['TOKEN']))) {
            ...
            if ($response['L_ERRORCODE0'] == 10417) {
                $errorText = MODULE_PAYMENT_PAYPALDP_TEXT_TRY_OTHER_PAYMENT_METHOD;
            }
     ...
     */
    public function clear_3DSecure_session_vars($thorough = false)
    {
        if ($thorough) {
            if (isset($_SESSION['3Dsecure_requires_lookup'])) {
                unset($_SESSION['3Dsecure_requires_lookup']);
            }
            if (isset($_SESSION['3Dsecure_card_type'])) {
                unset($_SESSION['3Dsecure_card_type']);
            }
        }
        if (isset($_SESSION['3Dsecure_merchantData'])) {
            unset($_SESSION['3Dsecure_merchantData']);
        }
        if (isset($_SESSION['3Dsecure_enroll_lookup_attempted'])) {
            unset($_SESSION['3Dsecure_enroll_lookup_attempted']);
        }
        if (isset($_SESSION['3Dsecure_authentication_attempted'])) {
            unset($_SESSION['3Dsecure_authentication_attempted']);
        }
        if (isset($_SESSION['3Dsecure_transactionId'])) {
            unset($_SESSION['3Dsecure_transactionId']);
        }
        if (isset($_SESSION['3Dsecure_enrolled'])) {
            unset($_SESSION['3Dsecure_enrolled']);
        }
        if (isset($_SESSION['3Dsecure_acsURL'])) {
            unset($_SESSION['3Dsecure_acsURL']);
        }
        if (isset($_SESSION['3Dsecure_payload'])) {
            unset($_SESSION['3Dsecure_payload']);
        }
        if (isset($_SESSION['3Dsecure_auth_status'])) {
            unset($_SESSION['3Dsecure_auth_status']);
        }
        if (isset($_SESSION['3Dsecure_sig_status'])) {
            unset($_SESSION['3Dsecure_sig_status']);
        }
        if (isset($_SESSION['3Dsecure_auth_xid'])) {
            unset($_SESSION['3Dsecure_auth_xid']);
        }
        if (isset($_SESSION['3Dsecure_auth_cavv'])) {
            unset($_SESSION['3Dsecure_auth_cavv']);
        }
        if (isset($_SESSION['3Dsecure_auth_eci'])) {
            unset($_SESSION['3Dsecure_auth_eci']);
        }
        if (isset($_SESSION['3Dsecure_term_url'])) {
            unset($_SESSION['3Dsecure_term_url']);
        }
        if (isset($_SESSION['3Dsecure_auth_url'])) {
            unset($_SESSION['3Dsecure_auth_url']);
        }
    }


    public function determine3DSecureProtection($cardType, $ECI)
    {
        ...
        } elseif (strcasecmp($cardType, "MASTERCARD") == 0) {
            if (strcasecmp($ECI, "02") == 0) {
                $resultStatus = "PROTECTED";
            } else {
                $resultStatus = "NOT PROTECTED";
            }
        } elseif (strcasecmp($cardType, "JCB") == 0) {
            if ((strcasecmp($ECI, "05") == 0) || (strcasecmp($ECI, "06") == 0)) {
                $resultStatus = "PROTECTED";
            } else {
                $resultStatus = "NOT PROTECTED";
     ...
     * @return array
     */
    public function get3DSecureLookupResponse($lookup_data_array)
    {
        // Set some defaults
     ...
     * @return array
     */
    public function get3DSecureAuthenticateResponse($authenticate_data_array)
    {
        ...
        if (isset($authenticate_data_array['merchantData'])) {
            $data .= '<MerchantData>' . $this->escapeXML($authenticate_data_array['merchantData']) . '</MerchantData>';
        }
        $data .= '</CardinalMPI>';
        ...
        if ($errorNo == 0) {
            if (strcasecmp($authStatus, 'Y') == 0 || strcasecmp($authStatus, 'A') == 0) {
                $continue_flag = 'Y';
            } elseif (strcasecmp($authStatus, 'N') == 0) {
                $continue_flag = 'N';
            } elseif (strcasecmp($authStatus, 'U') == 0) {
                if (strcasecmp(MODULE_PAYMENT_PAYPALDP_CARDINAL_AUTHENTICATE_REQ, 'No') == 0) {
    ...
    public function send3DSecureHttp($url, $data, $debugData)
    {
      ...
      // create a new cURL resource and set params
            ...
            } elseif (strpos($result, "<CardinalMPI>") === false) {
    ...
    public function getISOCurrency($curr)
    {
        $out = "";
    ...
    public function formatRawAmount($amount, $curr)
    {
    ...
    public function requiresLookup($info)
    {
        if (is_numeric($info)) {
...
class CardinalXMLParser
{
    ...
    public function CardinalXMLParser()
    {
