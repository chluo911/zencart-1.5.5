...
class queryFactory extends base
{
    ...
    public function query($link, $query, $remove_from_queryCache = false)
    {
        global $queryLog;
        global $queryCache;
        ...
        if (isset($queryLog)) {
            $queryLog->start($query);
        }
        $result = mysqli_query($link, $query);
        if (isset($queryLog)) {
            $queryLog->stop($query, $result);
        }
        if (isset($queryCache)) {
    ...
    public function connect($zf_host, $zf_user, $zf_password, $zf_database, $zf_pconnect = 'false', $dieOnErrors = false, $options = array())
    {
    ...
    public function simpleConnect($zf_host, $zf_user, $zf_password, $zf_database)
    {
        $this->database = $zf_database;
        $this->user = $zf_user;
        $this->host = $zf_host;
    ...
    public function set_error($zp_err_num, $zp_err_text, $dieOnErrors = true)
    {
    ...
    public function Execute($zf_sql, $zf_limit = false, $zf_cache = false, $zf_cachetime=0, $remove_from_queryCache = false)
    {
    ...
    public function ExecuteRandomMulti($zf_sql, $zf_limit = 0, $zf_cache = false, $zf_cachetime=0, $remove_from_queryCache = false)
    {
        $this->zf_sql = $zf_sql;
        $time_start = explode(' ', microtime());
        $obj = new queryFactoryResult($this->link);
    ...
    public function metaColumns($zp_table)
    {
    ...
    **/
    public function bindVars($sql, $bindVarString, $bindVarValue, $bindVarType, $debug = false)
    {
...
class queryFactoryResult implements Countable, Iterator
{
    ...
    */
    ...
    public function RecordCount()
    {
        ...
        } elseif ($this->resource !== null && $this->resource !== true) {
    ...
    public function Move($zp_row)
    {
        ...
        } elseif (@mysqli_data_seek($this->resource, $zp_row)) {
            $zp_result_array = @mysqli_fetch_array($this->resource);
            while (list($key, $value) = each($zp_result_array)) {
                $this->fields[$key] = $value;
            }
