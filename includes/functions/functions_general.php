...
// check to see if database stored GET terms are in the URL as $_GET parameters
  function zen_check_url_get_terms()
  {
      ...
      while (!$zp_filter_terms->EOF) {
          if (isset($_GET[$zp_filter_terms->fields['get_term_name']]) && zen_not_null($_GET[$zp_filter_terms->fields['get_term_name']])) {
              $zp_result = true;
          }
 ...
 */
  function charsetClean($string)
  {
      ...
      if (function_exists('iconv')) {
          $string = iconv("Windows-1252", CHARSET . "//IGNORE", $string);
      }
      $string = htmlentities($string, ENT_QUOTES, 'UTF-8');
  ...
  function utilDumpRequest($mode='p', $out = 'log')
  {
      ...
      if ($out == 'log' || $out == 'l') {
          error_log($val);
      } elseif ($out == 'die' || $out == 'd') {
          die($val);
      } elseif ($out == 'echo' || $out == 'e') {
          echo $val;
      }
  }
