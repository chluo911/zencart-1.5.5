...
<?php
    for ($col=0;$col<sizeof($list_box_contents[$row]);$col++) {
        ...
        if (isset($list_box_contents[$row][$col]['params'])) {
            $c_params .= ' ' . $list_box_contents[$row][$col]['params'];
        }
        if (isset($list_box_contents[$row][$col]['align']) && $list_box_contents[$row][$col]['align'] != '') {
            $c_params .= ' align="' . $list_box_contents[$row][$col]['align'] . '"';
        }
        if ($cell_type=='th') {
            $c_params .= ' scope="' . $cell_scope . '" id="' . $cell_title . 'Cell' . $row . '-' . $col.'"';
        }
