...
<br class="clearBoth" />
<?php if ($messageStack->size('gv_send') > 0) {
...
<?php echo zen_draw_input_field('email', $_POST['email'], 'size="40" id="email-address"', 'email') . '<span class="alert">' . ENTRY_REQUIRED_SYMBOL . '</span>';
      if ($error) {
          echo $error_email;
      } ?>
<br class="clearBoth" />
...
<?php echo zen_draw_input_field('amount', $_POST['amount'], 'id="amount"', 'text', false) . '<span class="alert">' . ENTRY_REQUIRED_SYMBOL . '</span>';
      if ($error) {
          echo $error_amount;
      } ?>
<br class="clearBoth" />
