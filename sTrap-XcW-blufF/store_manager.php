...
// update all products in catalog
    case ('update_all_products_attributes_sort_order'):
      if (isset($_POST['confirm']) && $_POST['confirm'] == 'yes') {
          $all_products_attributes= $db->Execute(
              "select p.products_id, pa.products_attributes_id from " .
        TABLE_PRODUCTS . " p, " .
    ...
    case ('update_counter'):
    if ($_POST['new_counter'] == '' || strval($_POST['new_counter']) != strval((int)$_POST['new_counter'])) {
        break;
    }
...
<?php if ($processing_message != '' && $processing_action_url != '') {
    echo '<meta http-equiv="refresh" content="2;URL=' . $processing_action_url . '">';
} ?>
