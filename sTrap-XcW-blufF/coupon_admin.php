    ...
    case 'update_confirm':
      if (($_POST['back_x']) || ($_POST['back_y'])) {
        ...
        if (substr($_POST['coupon_amount'], -1) == '%' && $_POST['coupon_free_ship']) {
            $coupon_type = 'E';
        } // percentage off and free shipping
        $_POST['coupon_amount'] = preg_replace('/[^0-9.]/', '', $_POST['coupon_amount']);
...
<?php
    ...
    while (!$cc_list->EOF) {
        ...
        $customer = $db->Execute("select customers_firstname, customers_lastname
                ...
                <td class="dataTableContent" align="right"><?php if ((is_object($cInfo)) && ($cc_list->fields['unique_id'] == $cInfo->unique_id)) {
            echo zen_image(DIR_WS_IMAGES . 'icon_arrow_right.gif');
        } else {
            echo '<a href="' . zen_href_link(FILENAME_COUPON_ADMIN, 'reports_page=' . $_GET['reports_page'] . '&cid=' . $cc_list->fields['coupon_id']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
        } ?>&nbsp;</td>
              </tr>
<?php
      $cc_list->MoveNext();
...
<?php
  ...
  case 'new':
// set some defaults
    if ($_GET['action'] != 'voucheredit' and $_POST['coupon_uses_user'] == '') {
        $_POST['coupon_uses_user'] = 1;
    }
?>
      <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
...
?>
      ...
      <tr>
        ...
        <td align="left"><input type="checkbox" name="coupon_free_ship" <?php if ($_POST['coupon_free_ship']) {
    echo 'CHECKED';
} ?>></td>
        ...
        default:
          ...
          break;
            ...
            &nbsp;</td>
                ...
                <td class="dataTableContent" align="right"><?php if ((is_object($cInfo)) && ($cc_list->fields['coupon_id'] == $cInfo->coupon_id)) {
          echo zen_image(DIR_WS_IMAGES . 'icon_arrow_right.gif');
      } else {
          echo '<a href="' . zen_href_link(FILENAME_COUPON_ADMIN, 'page=' . $_GET['page'] . '&cid=' . $cc_list->fields['coupon_id'] . (isset($_GET['status']) ? '&status=' . $_GET['status'] : '')) . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
      } ?>&nbsp;</td>
              </tr>
<?php
      $cc_list->MoveNext();
...
<?php
    ...
    default:
      ...
      if ($_GET['action'] == 'voucherdelete' or $_GET['action'] == 'vouchercopy') {
          ...
          //bof 12-6ke
          $product_query = $db->Execute("select * from " . TABLE_COUPON_RESTRICT . " where coupon_id = '" . $cInfo->coupon_id . "' and product_id != '0'");
