 ...
 */
  ...
  if (zen_not_null($action)) {
      switch ($action) {
      case 'edit':
        if (!is_writable(DIR_FS_CATALOG_MEDIA)) {
            $messageStack->add(TEXT_WARNING_FOLDER_UNWRITABLE, 'caution');
        }
      break;
      case 'remove_product':
...
<?php
  ...
  while (!$media->EOF) {
      ...
      if (isset($mInfo) && is_object($mInfo) && ($media->fields['media_id'] == $mInfo->media_id)) {
          ...
          echo '<a href="' . zen_href_link(FILENAME_MEDIA_MANAGER, zen_get_all_get_params(array('mID')) . 'mID=' . $media->fields['media_id']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
      } ?>
                </td>
              </tr>
<?php
...
<?php
  ...
  switch ($action) {
    ...
    case 'products':
      ...
      if ($products_linked->RecordCount() > 0) {
          $contents[] = array('text' => '<hr />');
      }
