...
//        } // endif $table_prefix
  ...
  function zen_drop_index_command($param)
  {
      ...
      while (!$result->EOF) {
          if (ZC_UPG_DEBUG3==true) {
              echo $result->fields['Key_name'].'<br />';
          }
  ...
  function zen_check_alter_command($param)
  {
      ...
      case ("ADD"):
        if (strtoupper($param[4]) == 'INDEX') {
            ...
            while (!$result->EOF) {
                if (ZC_UPG_DEBUG3==true) {
                    echo 'KEY: '.$result->fields['Key_name'].'<br />';
                }
                if ($result->fields['Key_name'] == $index) {
...
<?php if (isset($_GET['nogrants'])) {
    echo '<input type="hidden" id="nogrants" name="nogrants" value="'.(bool)$_GET['nogrants'].'" />';
} ?>
      <td><table border="0" cellpadding="0" cellspacing="2">
        <tr>
