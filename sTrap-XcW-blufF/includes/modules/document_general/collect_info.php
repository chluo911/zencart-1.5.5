...
// Virtual Products
    if (!isset($pInfo->products_virtual)) {
        $pInfo->products_virtual = DEFAULT_DOCUMENT_GENERAL_PRODUCTS_VIRTUAL;
    }
...
// Always Free Shipping
    if (!isset($pInfo->product_is_always_free_shipping)) {
        $pInfo->product_is_always_free_shipping = DEFAULT_DOCUMENT_GENERAL_PRODUCTS_IS_ALWAYS_FREE_SHIPPING;
    }
...
<?php
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ?>
          <tr>
            <td class="main"><?php if ($i == 0) {
            echo TEXT_DOCUMENT_URL . '<br /><small>' . TEXT_DOCUMENT_URL_WITHOUT_HTTP . '</small>';
        } ?></td>
