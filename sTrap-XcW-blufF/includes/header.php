...
if (file_exists(DIR_FS_ADMIN . 'includes/local/skip_version_check.ini')) {
    ...
    foreach ($lines as $line) {
        ...
        if (substr(trim($line), 0, 46) == 'display_update_link_on_index_and_sysinfo_page=') {
            $version_ini_index_sysinfo = trim(strtolower(str_replace('display_update_link_only_on_sysinfo_page=', '', $line)));
        }
    }
}
...
//          if ($new_version != '') $new_version .= '<br />';
    ...
    if ($new_version != '' && $new_version != TEXT_VERSION_CHECK_CURRENT) {
        $new_version .= '<br /><a href="' . $lines[6] . '" target="_blank"><input type="button" class="btn btn-success" value="' . TEXT_VERSION_CHECK_DOWNLOAD . '"/></a>';
    }
} else {
    ...
    if ($zv_db_patch_ok == true || $version_check_sysinfo == true) {
        $new_version = '<a href="' . $url . '">' . '<input type="button" class="btn btn-link" value="' . TEXT_VERSION_CHECK_BUTTON . '"/></a>';
    }
}
...
<!-- All HEADER_ definitions in the columns below are defined in includes/languages/english.php //-->
  <div class="row">
    ...
    <div class="col-xs-6 col-sm-3 col-sm-pull-3 noprint adminHeaderAlerts">
        <?php if ($new_gv_queue_cnt > 0) {
            echo $goto_gv . '<br />' . sprintf(TEXT_SHOW_GV_QUEUE, $new_gv_queue_cnt);
        } ?>
    </div>
...
<?php if (file_exists(DIR_WS_INCLUDES . 'keepalive_module.php')) {
