...
class zcObserverLogEventListener extends base
{
   ...
   */
    ...
    public static function prepareLogdata($message_to_log = '', $requested_severity = '')
    {
        global $PHP_SELF;
        $flagged = 0;
     ...
     */
    public static function filterArrayElements($data)
    {
        foreach ($data as $key=>$nul) {
            if (in_array($key, array('x','y','secur'.'ityTo'.'ken','admi'.'n_p'.'ass','pass'.'word','confirm', 'newpwd-'.$_SESSION['securityToken'],'oldpwd-'.$_SESSION['securityToken'],'confpwd-'.$_SESSION['securityToken']))) {
                unset($data[$key]);
            }
        }
        return $data;
     ...
     */
    public static function ensureDataIsUtf8($data)
    {
        ...
        foreach ($data as $key=>$nul) {
            if (is_string($nul)) {
                $data[$key] = utf8_encode($nul);
            }
            if (is_array($nul)) {
                foreach ($nul as $key2=>$val) {
                    if (is_string($val)) {
                        $data[$key][$key2] = utf8_encode($val);
                    }
                    if (is_array($val)) {
                        $data[$key][$key2] = utf8_encode(print_r($val, true));
                    }
                }
            }
        }
     ...
     */
    public static function parseForMaliciousContent($string)
    {
        $matches = '';
