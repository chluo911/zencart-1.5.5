  ...
  function zen_draw_products_pull_down($name, $parameters = '', $exclude = '', $show_id = false, $set_selected = false, $show_model = false, $show_current_category = false)
  {
      ...
      while (!$products->EOF) {
          if (!in_array($products->fields['products_id'], $exclude)) {
              ...
              if ($set_selected == $products->fields['products_id']) {
...
// Retreive server information
  function zen_get_system_information()
  {
      ...
      if (!$result->EOF) {
          ...
          if (strstr($result->fields['Value'], 'strict_')) {
              $strictmysql = true;
          }
      }
      ...
      if (!$exec_disabled) {
          ...
          if ($errnum == 0 && sizeof($output)) {
              list($system, $host, $kernel) = preg_split('/[\s,]+/', $output[0], 5);
          }
          $output = '';
 ...
 */
  function zen_get_products_options_name_from_value($lookup)
  {
      ...
      if ($check_options_to_values->EOF) {
          return '';
      }
 ...
 */
  function zen_get_handler_from_type($product_type)
  {
      ...
      if ($handler->EOF) {
          return 'ERROR: Invalid type_handler. Your product_type settings are wrong, incomplete, or damaged.';
      }
...
function zen_sort_array($data, $columnName1 = '', $order1 = SORT_ASC, $columnName2 = '', $order2 = SORT_ASC)
{
    ...
    if (!in_array($order1, array(SORT_ASC, SORT_DESC))) {
        $order1=SORT_ASC;
    }
    if ($columnName2 == '') {
        $columnName2 = $keys[1];
    }
    if (!in_array($order2, array(SORT_ASC, SORT_DESC))) {
        $order2=SORT_ASC;
    }
